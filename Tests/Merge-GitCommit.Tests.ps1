...
BeforeAll {
    ...
    $script:testNum = 0
    [Git.Automation.MergeResult]$script:result = $null

    function Get-RepoRoot
    {
        return Join-Path -Path $script:testDirPath -ChildPath 'repo'
    }
    ...
    function ThenCommitCountIs
    {
        ...
        $repoRoot = Get-RepoRoot
        Get-GitCommit -RepoRoot $repoRoot -All |
            Measure-Object |
    ...
    function ThenMergeStatus
    {
        ...
        {
            $script:result.Status | Should -Be $Is
        }
    }
    ...
    function ThenNotMerged
    {
        ...
        $repoRoot = Get-RepoRoot
        (Get-GitCommit -RepoRoot $repoRoot -Revision $Revision1).Sha |
            Should -Not -Be (Get-GitCommit -RepoRoot $repoRoot -Revision $Revision2).Sha
    }
...
Describe 'Merge-GitCommt' {
    ...
    It 'merging one branch into another and can fast forward' {
        GivenGitRepository
    ...
    It 'merging one branch into another and can''t fast forward' {
        GivenGitRepository
        GivenGitBranch 'develop'
        GivenFile 'develop'
        GivenCurrentHead 'master'
    ...
    It 'merging one branch into another and user doesn''t want to fast forward' {
        GivenGitRepository
        GivenGitBranch 'develop'
        GivenFile 'develop'
        GivenCurrentHead 'master'
    ...
    It 'merging one branch into another and user only wants to fast forward' {
        GivenGitRepository
        GivenGitBranch 'develop'
        GivenFile 'develop'
        GivenCurrentHead 'master'
    ...
    It 'merging one branch into another and user only wants to fast forward but can''t fast forward' {
        GivenGitRepository
        GivenGitBranch 'develop'
        GivenFile 'develop'
        GivenCurrentHead 'master'
    ...
    It 'there are conflicts and merging non-interactively' {
        GivenGitRepository
        GivenGitBranch 'develop'
        GivenFile 'conflict' 'develop'
        GivenCurrentHead 'master'
